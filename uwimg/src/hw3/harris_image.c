...
image structure_matrix(image im, float sigma)
{
    ...
    image R1 = convolve_image(im, gx, 0);
    image R2 = convolve_image(im, gy, 0);
    ...
    for(i = 0 ; i < size ; i++){
        ...
        s.data[i] = Ix*Ix;
        s.data[i + size] = Iy*Iy;
        s.data[i + 2*size] = Ix*Iy;

    }

    image S = smooth_image(s, sigma);
...
image nms_image(image im, int w)
{
    ...
    for(y = 0 ; y < im.h ; y++){
        ...
        for(x = 0 ; x < im.w ; x++){
            ...
            w_end = w_end < im.w ? w_end : im.w - 1 ;

                for(wy = h_start ; wy <= h_end ; wy++){
                    for (wx = w_start; wx <= w_end; wx++)
                    {   
