    ...
    for(k = 0 ; k < integ.c ; k++){
    	for( j = 0 ; j < integ.h ; j++){
	    	for(i = 0 ; i < integ.w ; i++){
                ...
                butx = i + r;
                buty = j + r;
                if(butx > im.w - 1) butx = im.w - 1;
                if(buty > im.h - 1) buty = im.h - 1;

    			if(topx >= 0 && topy >= 0) value += get_pixel(integ, topx, topy, k);    			
    			if(topx >= 0) value -= get_pixel(integ, topx, buty, k);
                if(topy >= 0) value -= get_pixel(integ, butx, topy, k);
                value += get_pixel(integ, butx, buty, k);
                value /= (topx - butx)*(topy - buty); 
...
image time_structure_matrix(image im, image prev, int s)
{
    ...
    image result = make_image(im.w, im.h, 5);
    ...
    for(i = 0 ; i < size ; i++){
    	...
    	x = Ix.data[i];
    	y = Iy.data[i];
    	dif = im.data[i] - prev.data[i];

    	result.data[i] = x * x;
    	result.data[i + size] = y * y;
    	result.data[i + 2 * size] = x * y;
    	result.data[i + 3 * size] = dif * x;
    	result.data[i + 4 * size] = dif * y; 

    }
    
    image S = box_filter_image(result, s);
